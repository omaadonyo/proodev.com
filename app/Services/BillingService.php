...
{
    /**
     * Create a pending developer verification payment. Two plans exist:
     * a $17 lifetime one-time purchase and an $8 monthly subscription.
     */
    public function createVerificationPayment(User $user, ?string $shortName = null, string $plan = 'lifetime'): Payment
    {
        if (! in_array($plan, ['lifetime', 'monthly'], true)) {
            throw new InvalidArgumentException('Unknown verification plan.');
        }
        ...
        return Payment::create([
            ...
            'amount' => (float) config(
                $monthly ? 'billing.developer.verification.monthly_price' : 'billing.developer.verification.lifetime_price',
                $monthly ? 8 : 17,
            ),
    ...
    private function fulfillVerification(Payment $payment): void
    {
        ...
        $plan = $payment->metadata['plan'] ?? 'lifetime';
        ...
        $user->update([
            ...
            // Monthly plans stay active until the paid window ends; paying
            // again extends it. Lifetime clears any previous expiry.
            'verification_expires_at' => $plan === 'monthly'
                ? now()->addDays((int) config('billing.developer.verification.interval_days', 30))
                : null,
        ]);
    }
