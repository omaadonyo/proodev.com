...
class SystemResetService
{
    ...
    /**
     * Run the reset. Operation is not wrapped in a transaction because some
     * drivers ignore `foreign_keys OFF` inside one; FKs are disabled for the
     * duration and re-enabled in a finally block so a mid-run failure never
     * leaves the connection in a lax state.
     *
    ...
    /**
     * Turn off foreign key enforcement for the active database driver.
     */
    ...
    /**
     * Restore foreign key enforcement for the active database driver.
     */
    ...
    /**
     * Reset auto-increment counters so new ids start near 1 again.
     */
        ...
        if (in_array(DB::getDriverName(), ['mysql', 'mariadb'], true)) {
            foreach ($this->purgeTables as $table) {
                try {
                    ...
                    // some tables have no auto-increment column — not worth failing the reset
                }
            }
        }
    }
