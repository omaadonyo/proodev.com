...
class ScoutRunner extends Component
{
    ...
    public function tick(): void
    {
        ...
        try {
            ...
            $this->phase = 'input';
            app(ScanEmailBatcher::class)->abandon(auth()->id() ?? 0);
        } catch (\Throwable $e) {
            \Log::error('ScoutRunner error: '.$e->getMessage(), [
                'exception' => $e,
                'url' => $this->url,
                'mode' => $this->mode,
                'step' => $this->step,
    ...
    private function beginProfileScan(): void
    {
        ...
        if ($user = auth()->user()) {
            app(ScanEmailBatcher::class)->begin($user, 'Scout');
        }
    ...
    private function beginSingleScout(): void
    {
        if ($user = auth()->user()) {
            ...
            $batcher->announce($user, 'Scanning '.$this->url, [$this->url]);
        }
        $this->plan = [
    ...
    private function runReposStep(): void
    {
        ...
        // First email: what is scanned with what is queued.
        if ($user = auth()->user()) {
            app(ScanEmailBatcher::class)->announce(
                $user,
                'Scanning @'.$this->result['handle']."'s public repositories",
                collect($repos)->pluck('name')->take(10)->all(),
                $this->queued > 0
                    ? [number_format($this->queued).' item'.($this->queued === 1 ? '' : 's').' queued for background scanning']
                    : [],
            );
        }
    ...
    private function runFetchStep(): void
    {
        ...
        $this->fillProfileFromSingleRepo();

        $this->completed[] = 'fetch';
    }

    /**
     * When scouting a single repository, extract headline/bio highlights and
     * hydrate the user's profile so the DevID Summary reads as highlights.
     */
    private function fillProfileFromSingleRepo(): void
    {
        $user = auth()->user();
        if (! $user || ! $this->material || ! $this->singleRepo) {
            return;
        }

        $material = $this->material;
        $repo = $this->singleRepo;
        $techStack = array_values(array_filter($material['tech_stack'] ?? []));
        $title = (string) ($material['title'] ?? $repo['name'] ?? '');
        $description = (string) ($material['description'] ?? $repo['description'] ?? '');
        $content = (string) ($material['content'] ?? '');

        $needsHeadline = blank($user->headline);
        $needsBio = blank($user->bio);
        $needsGithub = blank($user->github_url) && ($material['source'] ?? '') === 'github';

        if (! $needsHeadline && ! $needsBio && ! $needsGithub) {
            return;
        }
        ...
        if ($needsHeadline) {
            ...
            if ($primaryTech && $title) {
                ...
                $headline = Str::limit($description, 80);
            } else {
                $headline = $title ? 'Builder of '.$title : 'Software Engineer';
            }
            $user->headline = Str::limit($headline, 120);
        }

        // Build bio as highlights that will appear on DevID Summary.
        // Highlights = top tech + description + stars/forks + content excerpt.
        if ($needsBio) {
            $parts = [];
            if ($techStack !== []) {
                $parts[] = 'Builds with '.implode(', ', array_slice($techStack, 0, 3));
            }
            if ($description) {
                $parts[] = $description;
            }
            if (($material['stars'] ?? 0) > 0) {
                $parts[] = number_format((int) $material['stars']).' stars'.(($material['forks'] ?? 0) > 0 ? ' · '.number_format((int) $material['forks']).' forks' : '');
            }
            $excerpt = trim(preg_replace('/\s+/', ' ', strip_tags($content)) ?? '');
            if ($excerpt !== '') {
                $parts[] = Str::limit($excerpt, 280);
            }
            // Fallback highlights from repo topics/language
            if ($parts === [] && ($repo['topics'] ?? []) !== []) {
                $parts[] = 'Focus: '.implode(', ', array_slice($repo['topics'], 0, 4));
            }
        ...
        if ($needsGithub && isset($repo['html_url'])) {
            $user->github_url = $repo['html_url'];
        }

        if ($user->isDirty('headline') || $user->isDirty('bio') || $user->isDirty('github_url')) {
            $user->save();
            // Refresh passport preview so the builder sees the new headline/bio immediately.
            $this->passport['profile']['headline'] = $user->headline;
        }
    }
    ...
    private function runFinalizeStep(): void
    {
        ...
        // Last email: details on every scanned project.
        if ($user = auth()->user()) {
            app(ScanEmailBatcher::class)->complete($user);
        }
    }
