...
class extends Component
{
    ...
    public function tick(): void
    {
        ...
        try {
            ...
            $this->error = $e->getMessage();
            $this->phase = 'input';
            app(ScanEmailBatcher::class)->abandon(auth()->id());
        } catch (Throwable $e) {
            \Log::error('Onboarding scout error: '.$e->getMessage(), [
                'exception' => $e,
                'url' => $this->url ?? null,
                'source' => $this->source ?? null,
            ]);
            $this->error = 'Connection issue while scouting your profile. Please try again.';
            $this->phase = 'input';
            app(ScanEmailBatcher::class)->abandon(auth()->id());
        }
    }
    ...
    private function beginGithubScan(): void
    {
        ...
        app(ScanEmailBatcher::class)->begin(auth()->user(), 'Onboarding scan');
    ...
    private function beginLegacy(): void
    {
        ...
        app(ScanEmailBatcher::class)->begin(auth()->user(), 'Onboarding scan');
        app(ScanEmailBatcher::class)->announce(auth()->user(), 'Scanning '.$this->url, [$this->url]);
    ...
    private function tickGithub(): void
    {
        if ($this->step >= count($this->plan)) {
            ...
            app(ScanEmailBatcher::class)->complete(auth()->user());
    ...
    private function runReposStep(): void
    {
        ...
        app(ScanEmailBatcher::class)->announce(
            auth()->user(),
            'Scanning @'.$this->handle."'s public repositories",
