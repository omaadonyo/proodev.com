...
new #[Title('DevID')] class extends Component
{
    ...
    #[Computed]
    public function contributions(): \Illuminate\Support\Collection
    {
        $user = $this->user;
        $counts = collect();

        foreach ($user->projects()->where('status', 'published')->pluck('tech_stack') as $stack) {
            foreach ((array) $stack as $t) {
                $counts[$t] = ($counts[$t] ?? 0) + 3;
            }
        }

        foreach ($user->evidence()->where('status', 'ready')->pluck('metadata') as $meta) {
            foreach (($meta['material']['tech_stack'] ?? []) as $t) {
                $counts[$t] = ($counts[$t] ?? 0) + 2;
            }
        }

        foreach ($user->skills()->get() as $skill) {
            $counts[$skill->name] = ($counts[$skill->name] ?? 0) + ($skill->pivot->level ?? 1);
        }

        $top = $counts->sortDesc()->keys()->first();

        $since = collect([
            optional(\Illuminate\Support\Carbon::parse($user->created_at))->year,
            optional(\Illuminate\Support\Carbon::parse($user->evidence()->min('created_at')))->year,
            optional(\Illuminate\Support\Carbon::parse($user->projects()->whereNotNull('published_at')->min('published_at')))->year,
        ])->filter()->values()->min();

        $latest = $user->projects()->where('status', 'published')->orderByDesc('published_at')->first();

        $items = collect();

        if ($top) {
            $items->push(['label' => __('Builds with :tech', ['tech' => $top]), 'context' => $this->techContext($top)]);
        }

        $second = $counts->sortDesc()->keys()->skip(1)->first();
        if ($second && $second !== $top) {
            $items->push(['label' => __('Builds with :tech', ['tech' => $second]), 'context' => $this->techContext($second)]);
        }

        $third = $counts->sortDesc()->keys()->skip(2)->first();
        if ($third && $third !== $top && $third !== $second) {
            $items->push(['label' => __('Builds with :tech', ['tech' => $third]), 'context' => $this->techContext($third)]);
        }

        if ($since && $since > 2000) {
            $items->push(['label' => __('Since :year', ['year' => $since]), 'context' => null]);
        }

        $publishedCount = $user->projects()->where('status', 'published')->count();
        if ($publishedCount > 0) {
            $items->push(['label' => trans_choice(':count published project|:count published projects', $publishedCount, ['count' => $publishedCount]), 'context' => null]);
        }

        $evidenceCount = $user->evidence()->where('status', 'ready')->count();
        if ($evidenceCount > 0) {
            $items->push(['label' => trans_choice(':count evidence item|:count evidence items', $evidenceCount, ['count' => $evidenceCount]), 'context' => null]);
        }

        if ($latest) {
            $items->push(['label' => $latest->title, 'context' => $latest->tagline ? \Illuminate\Support\Str::limit($latest->tagline, 40) : null]);
        }

        return $items;
    }

    private function techContext(?string $name): ?string
    {
        $lc = strtolower($name ?? '');

        return match (true) {
            str_contains($lc, 'laravel') => __('Popular PHP framework'),
            str_contains($lc, 'livewire') => __('Full-stack Laravel framework'),
            str_contains($lc, 'alpine') => __('Lightweight JS framework'),
            str_contains($lc, 'vue') => __('Progressive JS framework'),
            str_contains($lc, 'react') || str_contains($lc, 'next') => __('JS UI library'),
            str_contains($lc, 'tailwind') => __('Utility-first CSS'),
            str_contains($lc, 'php') => __('Server-side language'),
            str_contains($lc, 'javascript') || $lc === 'js' => __('Client-side language'),
            str_contains($lc, 'typescript') || $lc === 'ts' => __('Typed JS language'),
            str_contains($lc, 'mysql') || str_contains($lc, 'postgres') || str_contains($lc, 'sqlite') => __('Database'),
            str_contains($lc, 'html') => __('Markup'),
            str_contains($lc, 'css') => __('Styling'),
            default => null,
        };
    }
}
