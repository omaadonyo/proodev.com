    ...
    <body class="min-h-screen overflow-x-clip bg-white text-zinc-900 antialiased selection:bg-zinc-900/20 dark:bg-zinc-950 dark:text-zinc-100">
        ...
        <section class="relative overflow-hidden border-t border-zinc-200 dark:border-white/5">
            <div class="relative mx-auto max-w-3xl px-4 py-20 text-center sm:px-6 lg:px-8">
                <h2 class="text-3xl font-bold tracking-tight text-zinc-900 sm:text-4xl dark:text-white">Someone Should Notice the <span class="text-gradient">Hard Problems You Solve.</span></h2>
                <div class="mt-8 space-y-2 text-left text-lg leading-relaxed text-zinc-600 dark:text-zinc-400">
                    <p>Maybe it was the bug that took two days to understand.</p>
                    <p>The performance issue nobody could find.</p>
                    <p>The pull request that finally fixed it.</p>
                    <p>The open-source project you quietly helped improve.</p>
                    <p>The system you built from scratch.</p>
                </div>
                <p class="mt-8 text-lg font-medium text-zinc-700 dark:text-zinc-300">These moments are part of who you are as an engineer.</p>
                <p class="mt-3 text-2xl font-bold text-zinc-900 dark:text-white">ProoDev helps make them visible.</p>
            </div>
        </section>
                                    ...
                                    <ul class="mt-1.5 grid gap-1 text-xs text-zinc-600 dark:text-zinc-300">
                                        @foreach (['Concurrency', 'Debugging', 'Database Systems', 'Testing'] as $skill)
                                            <li class="flex items-center gap-1.5">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-3.5 shrink-0 text-emerald-500"><path fill-rule="evenodd" d="{{ $icon['check'] }}" clip-rule="evenodd"/></svg>
                                                {{ $skill }}
                                            </li>
                                        @endforeach
                                    </ul>
                ...
                <div class="mx-auto mt-14 grid max-w-4xl items-stretch gap-5 lg:grid-cols-2">
                    <div class="rounded-xl border border-zinc-200 bg-white p-5 dark:border-white/10 dark:bg-zinc-950/60">
                        ...
                        <div class="mt-3 rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-white/10 dark:bg-zinc-900/70">
                            ...
                            <ul class="mt-2 grid gap-1 text-xs text-zinc-600 dark:text-zinc-300">
                                <li class="flex items-center gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-3.5 text-emerald-500"><path fill-rule="evenodd" d="{{ $icon['check'] }}" clip-rule="evenodd"/></svg> 8 projects</li>
                                <li class="flex items-center gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-3.5 text-emerald-500"><path fill-rule="evenodd" d="{{ $icon['check'] }}" clip-rule="evenodd"/></svg> 4 open-source contributions</li>
                                <li class="flex items-center gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-3.5 text-emerald-500"><path fill-rule="evenodd" d="{{ $icon['check'] }}" clip-rule="evenodd"/></svg> 3 architecture achievements</li>
                                <li class="flex items-center gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-3.5 text-emerald-500"><path fill-rule="evenodd" d="{{ $icon['check'] }}" clip-rule="evenodd"/></svg> Verified Laravel expertise</li>
                            </ul>
                        </div>
                    </div>
        ...
        <section class="relative overflow-hidden border-t border-zinc-200 bg-zinc-50 dark:border-white/5 dark:bg-white/[0.02]">
            <div class="relative mx-auto max-w-3xl px-4 py-20 text-center sm:px-6 lg:px-8">
                <h2 class="text-3xl font-bold tracking-tight text-zinc-900 sm:text-4xl dark:text-white">You Don't Need Thousands of Followers to Be a Great Engineer.</h2>
                <p class="mt-6 text-lg leading-relaxed text-zinc-600 dark:text-zinc-400">A developer can make an important contribution without having a huge audience. ProoDev focuses on the work itself.</p>
                <div class="mt-8 flex flex-wrap items-center justify-center gap-2">
                    @foreach (['Difficult problems', 'Quality contributions', 'Open source', 'Architecture', 'Performance', 'Security', 'Documentation', 'Technical innovation'] as $focus)
                        <span class="rounded-full border border-zinc-200 bg-white px-4 py-2 text-sm font-medium text-zinc-700 dark:border-white/10 dark:bg-white/5 dark:text-zinc-200">{{ $focus }}</span>
                    @endforeach
                </div>
            </div>
        </section>
