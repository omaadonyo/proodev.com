...
@php
    $links = collect([
        ...
        ['label' => 'Website', 'url' => $user->website_url, 'icon' => 'globe'],
    ...
    <div {{ $attributes->merge(['class' => 'flex flex-wrap items-center gap-1.5']) }}>
        @foreach ($links as $link)
            <a
                ...
                target="_blank"
                rel="noopener"
                class="inline-flex items-center gap-1.5 rounded-full bg-zinc-100 px-2.5 py-1 text-xs font-medium text-zinc-600 transition hover:bg-zinc-200 hover:text-zinc-900 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700 dark:hover:text-white"
            >
                @if ($link['icon'] === 'github')
                ...
                @else
                    <flux:icon name="globe" variant="micro" />
                @endif
                {{ $link['label'] }}
            </a>
        @endforeach
