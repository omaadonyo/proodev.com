...
<body>
    <div class="wrapper">
        <div class="container {{ ($wide ?? false) ? 'container--wide' : '' }}">
            ...
            <div class="footer">
                <div class="brand-line">{{ str_replace(['https://', 'http://'], '', config('billing.seller.website')) }} | {{ config('billing.seller.name') }}</div>
                {{ config('billing.seller.address') }}, {{ config('billing.seller.city') }} - {{ config('billing.seller.country') }}<br>
                Tel: {{ config('billing.seller.phone') }} · <a href="mailto:{{ config('billing.seller.email') }}">{{ config('billing.seller.email') }}</a> · Tax ID {{ config('billing.seller.tax_id') }}
